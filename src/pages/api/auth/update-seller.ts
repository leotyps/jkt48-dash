...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const checkUserQuery = `SELECT id, seller FROM users WHERE phone_number = $1;`;
    ...
    let currentStatus = checkUserResult.rows[0].is_seller;

    // Jika status tidak berubah, langsung return
    if (currentStatus === newStatus) {
      return res.status(200).json({ message: 'No changes made, status is already set', is_seller: currentStatus });
    }

    // Update status seller pengguna
    const updateQuery = `
      UPDATE users
      SET seller = $1
    ...
    res.status(200).json({
      ...
      new_status: updateResult.rows[0].is_seller
    });
